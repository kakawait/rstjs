define([
    ...
    'rstjs/parser/error',
    'util'
], function (Core, StateMachine, Grammar, Unicode, LineReader, Tree, Nodes, Validator, ParserError, Util) {
    'use strict';
    ...
    Inliner.prototype.inline = function(type, match, lastPosition, final) {
        ...
        if (!this.quoted(match)) {
            ...
            if ((result = pattern.exec(source)) && final.lookBehind(source, result.index)) {
                var value = this.unescape(source.substring(0, result.index), true);
                return {
    ...
    Inliner.prototype.emphasis = function(match, lastPosition) {
        var type = 'emphasis';
        return this.inline(type, match, lastPosition, Grammar.inline.final[type]);
    };
    Inliner.prototype.strong = function(match, lastPosition) {
        var type = 'strong';
        return this.inline(type, match, lastPosition, Grammar.inline.final[type]);
    };
    Inliner.prototype.literal = function(match, lastPosition) {
        var type = 'literal';
        return this.inline(type, match, lastPosition, Grammar.inline.final[type]);
    };
