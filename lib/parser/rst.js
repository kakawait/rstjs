define([
    ...
    /*(?P<initial_inline>(?P<start>\\*\\*|\\*(?!\\*)|``|_`|\\|(?!\\|))(?![ \\n])|(?P<whole>(?P<refname>(?:(?!_)\\w)+(?:[-._+:](?:(?!_)\\w)+)*)(?P<refend>__?)|\\[(?P<footnotelabel>[0-9]+|\\#((?:(?!_)\\w)+(?:[-._+:](?:(?!_)\\w)+)*)?|\\*|(?P<citationlabel>(?:(?!_)\\w)+(?:[-._+:](?:(?!_)\\w)+)*))(?P<fnend>\\]_))|(?P<role>(:(?:(?!_)\\w)+(?:[-._+:](?:(?!_)\\w)+)*:)?)(?P<backquote>`(?!`))(?![ \\n]))*/
    // start: 
    //     (?P<start>
    //         \\*\\*|\\*(?!\\*)|``|_`|\\|(?!\\|)
    //     )
    ...
    */
    ...
    function Inliner() {
        ...
        //console.log(this.initial.source);
    ...
    Inliner.prototype.phraseRef = function(source) {
        ...
        console.log(match.input.substring(0, -2));
...
/*
Body => line        =======================
Line => text        Authentication adapters
Body => blank       
Body => text        test ``literal`` ``test``
Text => underline       -------------------------
Body => blank       
{ title: null,
  children: 
   [ { type: 'header',
       value: 'Authentication adapters',
       children: [],
       level: 1 },
     { type: 'header',
       value: 'test ``literal`` ``test``',
       children: [],
       level: 2 } ] }
*/